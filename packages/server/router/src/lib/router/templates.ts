...
import { z } from 'zod'
import { type Prisma, prismaPortal, TemplateType } from '@magickml/portal-db'
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (isUserRequested) {
        if (isUser) {
          ...
          where.public = undefined
        } else {
          where.type = TemplateType.COMMUNITY
          where.public = true
        }
      } else {
        where.type = input.type
        where.public = input.type === TemplateType.OFFICIAL ? true : undefined
      }
